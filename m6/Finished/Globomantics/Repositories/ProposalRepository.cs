...
public class ProposalRepository : IProposalRepository
{
    private static List<ProposalModel> proposals = new() {
        new ProposalModel { Id = 1, ConferenceId = 1, Title = "Authentication and Authorization in ASP.NET Core", Speaker = "Roland" },
    ...
    public void EditProposal(ProposalModel proposal)
    {
        var existingProposal = proposals.Single(p => p.Id == proposal.Id);
        existingProposal.Title = proposal.Title;
        existingProposal.Speaker = proposal.Speaker;
    }
